...
void                _SESSION_UNLOCK(struct session * sess);
...
task_t              _PROC_TASK(proc_t p);
int                 _FILL_VNODEINFO(vnode_t vp, struct vnode_info *vinfo, __unused boolean_t check_fsgetpath);
void                _FILL_FILEINFO(struct fileproc *fp, proc_t proc, int fd, struct proc_fileinfo * finfo);
errno_t             _FILL_SOCKETINFO(socket_t so, struct socket_info *si);
...
int
_PROC_PIDBSDINFO(proc_t p, struct proc_bsdinfo * pbsd, int zombie)
{
...
int
_PROC_PIDTASKINFO(proc_t p, struct proc_taskinfo * ptinfo)
{
...
int
_PROC_PIDTHREADINFO(proc_t p, uint64_t arg, bool thuniqueid, struct proc_threadinfo *pthinfo)
{
...
int
_PROC_GETTTY(proc_t p, vnode_t *vp)
{
    _proc_gettty = (void*)lookup_symbol("_proc_gettty");

    return _proc_gettty(p, vp);
}
...
void
_FILL_FILEINFO(struct fileproc *fp, proc_t proc, int fd, struct proc_fileinfo * finfo)
{
...
errno_t
_FILL_SOCKETINFO(socket_t so, struct socket_info *si)
{
...
kern_return_t
_TASK_THREADS(task_t task, thread_act_array_t *threads_out, mach_msg_type_number_t *count)
{
...
kern_return_t
_THREAD_INFO(thread_t thread, thread_flavor_t flavor, thread_info_t thread_info, mach_msg_type_number_t *thread_info_count)
{
...
thread_t
_CONVERT_PORT_TO_THREAD(ipc_port_t port)
{
