...
#define kalloc_type(type, flags) \
	((type *)_MALLOC(sizeof(type), M_TEMP, ((flags) & Z_ZERO) ? (M_WAITOK | M_ZERO) : M_WAITOK))
#define kalloc_data(size, flags) \
	_MALLOC((size_t)(size), M_TEMP, ((flags) & Z_ZERO) ? (M_WAITOK | M_ZERO) : M_WAITOK)
#define kfree_data(addr, size)  _FREE((void *)(addr), M_TEMP)
#define kfree_type(type, addr)  _FREE((void *)(addr), M_TEMP)
