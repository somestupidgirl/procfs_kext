...
static int (*_kauth_cred_issuser)(kauth_cred_t _cred);
...
procfs_check_can_access_process(kauth_cred_t creds, proc_t p) {
    ...
    if (cred_euid == kauth_cred_getuid(pcred) || cred_euid == kauth_cred_getruid(pcred)) {
        return 0;
    }
    ...
    if (cred_egid == kauth_cred_getgid(pcred) || cred_egid == kauth_cred_getruid(pcred)) {
        return 0;
    }
