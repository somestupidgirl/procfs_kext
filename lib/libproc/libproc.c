...
{
	proc_info = SymbolLookup("_proc_info");

	int errno;
	int retval;

	if ((type >= PROC_ALL_PIDS) || (type <= PROC_PPID_ONLY)) {
		if ((retval = proc_info(PROC_INFO_CALL_LISTPIDS, type, typeinfo, (uint64_t)0, buffer, buffersize)) == -1) {
			return 0;
		}
	} else {
		errno = EINVAL;
...
{
	proc_info = SymbolLookup("_proc_info");
	getpid = SymbolLookup("_getpid");

	int retval;

	if ((retval = proc_info(PROC_INFO_CALL_PIDORIGINATORINFO, getpid(), flavor, 0, buffer, buffersize)) == -1) {
		return 0;
	}
...
proc_listcoalitions(int flavor, int coaltype, void *buffer, int buffersize)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_LISTCOALITIONS, flavor, coaltype, 0, buffer, buffersize)) == -1) {
		return 0;
	}
...
proc_pid_rusage(int pid, int flavor, rusage_info_t *buffer)
{
	...
	return proc_info(PROC_INFO_CALL_PIDRUSAGE, pid, flavor, 0, buffer, 0);
}

int
proc_setthread_cpupercent(uint8_t percentage, uint32_t ms_refill)
{
	proc_rlimit_control = SymbolLookup("_proc_rlimit_control");
...
proc_pidfdinfo(int pid, int fd, int flavor, void * buffer, int buffersize)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_PIDFDINFO, pid, flavor, (uint64_t)fd, buffer, buffersize)) == -1) {
		return 0;
	}
...
proc_pidfileportinfo(int pid, uint32_t fileport, int flavor, void *buffer, int buffersize)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_PIDFILEPORTINFO, pid, flavor, (uint64_t)fileport, buffer, buffersize)) == -1) {
		return 0;
	}
	return retval;
}
...
proc_piddynkqueueinfo(int pid, int flavor, kqueue_id_t kq_id, void *buffer, int buffersize)
{
	...
	if ((ret = proc_info(PROC_INFO_CALL_PIDDYNKQUEUEINFO, pid, flavor, (uint64_t)kq_id, buffer, buffersize)) == -1) {
		return 0;
	}
...
proc_udata_info(int pid, int flavor, void *buffer, int buffersize)
{
	...
	return proc_info(PROC_INFO_CALL_UDATA_INFO, pid, flavor, 0, buffer, buffersize);
}

int
proc_bsd_name(int pid, void * buffer, uint32_t buffersize)
{
	int errno;
	int retval = 0, len;
...
proc_kmsgbuf(void * buffer, uint32_t  buffersize)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_KERNMSGBUF, 0, 0, (uint64_t)0, buffer, buffersize)) == -1) {
		return 0;
	}
	return retval;
}
...
proc_pidpath(int pid, void * buffer, uint32_t  buffersize)
{
	...
	retval = proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDPATHINFO, (uint64_t)0, buffer, buffersize);
	if (retval != -1) {
		len = (int)strlen(buffer);
		return len;
	}
...
{
	proc_info_extended_id = SymbolLookup("_proc_info_extended_id");
	...
	retval = proc_info_extended_id(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDPATHINFO, PIF_COMPARE_IDVERSION, (uint64_t)idversion,
...
proc_setpcontrol(const int control)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_PCONTROL, (uint64_t)control, NULL, 0)) == -1) {
		return errno;
	}
...
proc_setthreadname(void * buffer, int buffersize)
{
	...
	retval = proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_THREADNAME, (uint64_t)0, buffer, buffersize);
...
proc_track_dirty(pid_t pid, uint32_t flags)
{
	...
	if (proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_TRACK, flags, NULL, 0) == -1) {
		return errno;
	}
...
proc_set_dirty(pid_t pid, bool dirty)
{
	...
	if (proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_SET, dirty, NULL, 0) == -1) {
		return errno;
	}
...
proc_get_dirty(pid_t pid, uint32_t *flags)
{
	...
	retval = proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_GET, 0, NULL, 0);
	if (retval == -1) {
		return errno;
	}
...
proc_clear_dirty(pid_t pid, uint32_t flags)
{
	...
	if (proc_info(PROC_INFO_CALL_DIRTYCONTROL, pid, PROC_DIRTYCONTROL_CLEAR, flags, NULL, 0) == -1) {
		return errno;
	}
...
proc_terminate(pid_t pid, int *sig)
{
	...
	retval = proc_info(PROC_INFO_CALL_TERMINATE, pid, 0, 0, NULL, 0);
	if (retval == -1) {
		return errno;
	}
...
{
	process_policy = SymbolLookup("_process_policy");
	...
	return process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_SET, PROC_POLICY_RESOURCE_USAGE,
...
proc_get_cpumon_params(pid_t pid, int *percentage, int *interval)
{
	...
	ret = process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_GET, PROC_POLICY_RESOURCE_USAGE,
	    PROC_POLICY_RUSAGE_CPU, (proc_policy_attribute_t*)&attr, pid, 0);
...
proc_resume_cpumon(pid_t pid)
{
	...
	return process_policy(PROC_POLICY_SCOPE_PROCESS,
...
proc_list_uptrs(int pid, uint64_t *buf, uint32_t bufsz)
{
	...
	return proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDLISTUPTRS, 0,
	           buf, bufsz);
}
...
proc_list_dynkqueueids(int pid, kqueue_id_t *buf, uint32_t bufsz)
{
	...
	return proc_info(PROC_INFO_CALL_PIDINFO, pid, PROC_PIDLISTDYNKQUEUES, 0,
	           buf, bufsz);
}
...
proc_setcpu_percentage(pid_t pid, int action, int percentage)
{
	...
	attr.ppattr_cpu_percentage = percentage;
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU, (proc_policy_attribute_t*)&attr, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_clear_cpulimits(pid_t pid)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_RESTORE, PROC_POLICY_RESOURCE_USAGE, PROC_POLICY_RUSAGE_CPU, NULL, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_appstate(int pid, int * appstatep)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_GET, PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_STATE, (proc_policy_attribute_t*)&state, pid, (uint64_t)0) != -1) {
		if (appstatep != NULL) {
			*appstatep = state;
		}
		return 0;
...
proc_setappstate(int pid, int appstate)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_STATE, (proc_policy_attribute_t*)&state, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_devstatusnotify(int devicestatus)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_DEVSTATUS, (proc_policy_attribute_t*)&state, getpid(), (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_pidbind(int pid, uint64_t threadid, int bind)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_APP_LIFECYCLE, PROC_POLICY_APPLIFE_PIDBIND, (proc_policy_attribute_t*)&state, passpid, threadid) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_can_use_foreground_hw(int pid, uint32_t *reason)
{
	...
	return proc_info(PROC_INFO_CALL_CANUSEFGHW, pid, 0, 0, reason, sizeof(*reason));
}
...
proc_donate_importance_boost()
{
	...
	rval = process_policy(PROC_POLICY_SCOPE_PROCESS,
...
proc_clear_vmpressure(pid_t pid)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_RESTORE, PROC_POLICY_RESOURCE_STARVATION, PROC_POLICY_RS_VIRTUALMEM, NULL, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_set_owner_vmpressure(void)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_VMRSRCOWNER, (uint64_t)0, NULL, 0)) == -1) {
		return errno;
	}
...
proc_set_delayidlesleep(void)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_DELAYIDLESLEEP, (uint64_t)1, NULL, 0)) == -1) {
		return errno;
	}
...
proc_clear_delayidlesleep(void)
{
	...
	if ((retval = proc_info(PROC_INFO_CALL_SETCONTROL, getpid(), PROC_SELFSET_DELAYIDLESLEEP, (uint64_t)0, NULL, 0)) == -1) {
		return errno;
	}
...
proc_disable_apptype(pid_t pid, int apptype)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_DISABLE, PROC_POLICY_APPTYPE, apptype, NULL, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_enable_apptype(pid_t pid, int apptype)
{
	...
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_ENABLE, PROC_POLICY_APPTYPE, apptype, NULL, pid, (uint64_t)0) != -1) {
		return 0;
	} else {
		return errno;
	}
...
proc_set_no_smt(void)
{
	...
	int errno;
	if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_NO_SMT, 0, NULL, getpid(), (uint64_t)0) == -1) {
		return errno;
	}
	return 0;
}
...
{
	process_policy = SymbolLookup("_process_policy");
	thread_selfid = SymbolLookup("_thread_selfid");

	int errno;
	if (process_policy(PROC_POLICY_SCOPE_THREAD, PROC_POLICY_ACTION_APPLY, PROC_POLICY_NO_SMT, 0, NULL, 0, thread_selfid()) == -1) {
		return errno;
	}
	return 0;
}
...
proc_set_csm(uint32_t flags)
{
	...
	if (flags & (PROC_CSM_TECS | PROC_CSM_ALL)) {
		if (process_policy(PROC_POLICY_SCOPE_PROCESS, PROC_POLICY_ACTION_APPLY, PROC_POLICY_TECS, 0, NULL, getpid(), (uint64_t)0) == -1) {
			return errno;
		}
	}
...
proc_setthread_csm(uint32_t flags)
{
	...
	if (flags & (PROC_CSM_TECS | PROC_CSM_ALL)) {
		if (process_policy(PROC_POLICY_SCOPE_THREAD, PROC_POLICY_ACTION_APPLY, PROC_POLICY_TECS, 0, NULL, 0, thread_selfid()) == -1) {
			return errno;
		}
	}
