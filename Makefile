...
tools:
	@mkdir -p $(OUT)
	$(MAKE) -C tools
	mv tools/procfs_ksyms $(OUT)
	mv tools/procfs_ksyms.dSYM $(OUT)
	mv tools/procfsd $(OUT)
	mv tools/procfsd.dSYM $(OUT)
...
.PHONY: all debug release install install-only clean tests tools tools-install